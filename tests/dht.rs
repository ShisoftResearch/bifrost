...
fn primary() {
    ...
    let dht1_server_node_changes_count = Arc::new(AtomicUsize::new(0));
    let dht1_server_node_changes_count_clone = Arc::new(AtomicUsize::new(0)).clone();
    dht1.watch_server_nodes_range_changed(&server_2, move |r| {
        println!("---------------->");
        dht1_server_node_changes_count_clone.fetch_add(1, Ordering::Relaxed);
    });

    let dht2_server_node_changes_count = Arc::new(AtomicUsize::new(0));
    let dht2_server_node_changes_count_clone = Arc::new(AtomicUsize::new(0)).clone();
    dht2.watch_server_nodes_range_changed(&server_2, move |r| {
        println!("---------------->");
        dht2_server_node_changes_count_clone.fetch_add(1, Ordering::Relaxed);
    });

    let dht3_server_node_changes_count = Arc::new(AtomicUsize::new(0));
    let dht3_server_node_changes_count_clone = Arc::new(AtomicUsize::new(0)).clone();
    dht3.watch_server_nodes_range_changed(&server_2, move |r| {
        println!("---------------->");
        dht3_server_node_changes_count_clone.fetch_add(1, Ordering::Relaxed);
    });
    ...
    assert_eq!(dht_1_mapping.get(&server_2).unwrap(), &12001);
    assert_eq!(dht_1_mapping.get(&server_3).unwrap(), &17999);
    ...
    assert_eq!(dht_2_mapping.get(&server_2).unwrap(), &30000);

    for i in 0..30000 {
        let k = format!("k - {}", i);
        assert!(dht3.get_server_by_string(&k).is_none()); // no member
    }

//    wait();
//    wait();
//    assert_eq!(dht1_server_node_changes_count.load(Ordering::Relaxed), 1);
//    assert_eq!(dht2_server_node_changes_count.load(Ordering::Relaxed), 1);
//    assert_eq!(dht3_server_node_changes_count.load(Ordering::Relaxed), 0);
}

...
use bifrost::raft::{client::RaftClient, Options, RaftService, Storage, DEFAULT_SERVICE_ID};
...
async fn test_single_node_cluster_recovery_becomes_leader() {
    ...
    // Phase 1: Create and run a single-node cluster with disk storage
    {
        ...
        assert!(
            initial_leader_id != 0,
            "Phase 1: Should have valid leader ID"
        );

        println!(
            "Phase 1: Leader ID = {}, Server ID = {}",
            initial_leader_id, raft_service.id
        );

        // Perform some operations to generate logs
        let client = RaftClient::new(&vec![address.clone()], DEFAULT_SERVICE_ID)
            .await
            .unwrap();
    ...
    // Phase 2: Restart the server with same storage - THIS IS THE BUG FIX TEST
    {
        ...
        println!(
            "Phase 2: Leader ID = {}, Server ID = {}",
            leader_id2, raft_service2.id
        );

        assert!(
            is_leader,
            "Phase 2: CRITICAL - Should be leader after recovery (single-node cluster)"
        );
        assert!(
            leader_id2 != 0,
            "Phase 2: Should have valid leader ID after recovery"
        );
        assert_eq!(
            leader_id2, raft_service2.id,
            "Phase 2: Should be its own leader"
        );
...
async fn test_single_node_multiple_restart_cycles() {
    ...
    for cycle in 1..=3 {
        ...
        assert!(
            ...
            "Cycle {}: Should be leader",
            cycle
        );
        let leader_id = raft_service.leader_id().await;
        assert!(
            leader_id != 0,
            "Cycle {}: Should have valid leader ID",
            cycle
        );
        ...
        let num_logs = raft_service.num_logs().await;
        println!(
            "Cycle {}: ✅ Leader elected successfully, {} logs",
            cycle, num_logs
        );

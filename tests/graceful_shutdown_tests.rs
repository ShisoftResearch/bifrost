...
async fn test_tcp_server_shutdown_releases_port() {
    ...
    let handle = tokio::spawn(async move {
        tcp_server_clone
            .listen(&addr_clone, Arc::new(|data| Box::pin(async move { data })))
            .await
            .unwrap();
    });
    ...
    let connect_result = timeout(Duration::from_secs(2), TcpStream::connect(&address)).await;
    assert!(
    ...
    let handle2 = tokio::spawn(async move {
        let result = tcp_server2_clone
            .listen(&addr_clone2, Arc::new(|data| Box::pin(async move { data })))
            .await;
        assert!(
            result.is_ok(),
            "Should be able to bind to the same port after shutdown"
        );
    });
    ...
    let connect_result2 = timeout(Duration::from_secs(2), TcpStream::connect(&address)).await;
    assert!(
        connect_result2.is_ok(),
        "Should be able to connect to new server on same port"
    );
...
async fn test_rpc_server_shutdown_releases_port() {
    ...
    assert!(
        connect_result.is_ok(),
        "Should be able to connect to RPC server"
    );
    ...
    assert!(
        connect_result2.is_ok(),
        "Should be able to connect to new RPC server on same port"
    );
...
async fn test_raft_service_shutdown_stops_tasks() {
    ...
    // Use scope to ensure proper cleanup
    {
        ...
        assert!(
            !raft_service.is_leader(),
            "Should not be leader after shutdown"
        );
...
async fn test_active_connections_close_on_shutdown() {
    ...
    let new_connect = timeout(Duration::from_secs(1), TcpStream::connect(&address)).await;
    assert!(
